...
def connect():
    ...
    # Issue with College VM
    # Used Neo4J on Laptop.
    # Saved password as neo4j1 not neo4j
    # This may need to be corrected with the line below
...
def get_Neo_Results(DEPTID):
    ...
    with driver.session() as session:
        try:
            ...
            if (len(values) == 0):
                logging.info("No one manages " + DEPTID)
                return true
            else:
                for x in values:
                    return x
